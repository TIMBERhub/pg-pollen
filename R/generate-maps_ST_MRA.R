...
# load the preds data
preds <- readRDS(here::here("output", paste0('polya-gamma-predictions_', version, '_MRA.RDS')))
...
for (species_to_plot in unique(dat_pi_mean$species)) {
    ...
    if (!file.exists(here::here("figures", "MRA", paste0("predictions-MRA-", species_to_plot, ".png")))) {
        p_mean <- dat_pi_mean %>%
        ...
        ggsave(p_mean / p_sd, 
               file = here::here("figures", "MRA", paste0("predictions-MRA-", species_to_plot, "-version-", version, ".png")), 
               height = 9,
               width = 16)
    }
}
